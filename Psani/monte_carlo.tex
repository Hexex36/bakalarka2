...
\begin{equation}
  C = e^{-rT} \frac{1}{N} \sum_{i=1}^N max(0, K - S_i).
\end{equation}
...
Metoda kontrolní proměnné využívá podobné opce, u které máme známou přesnou cenu opce (skrze Black-Scholes či jiné metody).
Pro obě opce vytvoříme odhad na stejné sadě náhodně generovaných čísel. Odhad ceny opce, kterou chceme ocenit označíme $V_1^*$, odhad ceny opce, u které známe cenu označíme $V_2^*$ a cenu této opce označíme $V_2$. Potom můžeme vytvořit alternativní odhad ceny opce, označený $V_1$, jako:

\begin{equation}
  V_1 = V_1^* - V_2^* + V_2
\end{equation}

Kvůli stejné kvalitě/náhodnosti generovaných čísel u $V_1^*$ a $V_2^*$, můžeme očekávat, že chyba u odhadu opce s již známou cenou je stejný jako u odhadu opce bez známé ceny.
...
Metoda antitetických proměnných nás nechává z jedné sady náhodných čísel vygenerovat dva odhady opčních cen.

Mějme $z_1, z_2, \dots, z_N$ takové, že $z_i$ je náhodná veličina v odhadu $S_i$. První odhad opčních cen vypočteme normálně z generovaných náhodných čísel. Druhý odhad můžeme získat aditivní inverzí náhodné veličiny pro každý vygenerovaný odhad vývoje ceny, čili:

\begin{equation}
  z^*_i = -z_i,
\end{equation}

čímž efektivně získáme vývoj o stejné síle, s opačným směřováním (růst místo poklesu, a naopak). Následně můžeme vypočíst odhad ceny opce z potenciálních vývojů ceny $S^*_1, S^*_2, \dots, S^*_N$, a tyto odhady zprůměrovat. 
