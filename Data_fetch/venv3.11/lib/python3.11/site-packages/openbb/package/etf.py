"""Autogenerated OpenBB /etf Module."""
...
>>> obb.etf.countries(symbol='VT')
    ...
    def equity_exposure(
        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    'Symbol to get data for. (underlying equity)'
                    'Multiple comma separated items allowed for'
                    'provider(s): fmp.'
                )
            )
        ],
...
provider : str
    The provider to use, by default None. If None, the priority list configured in the settings is used. Default priority: fmp.
symbol : str | list[str]
    Symbol to get data for. (underlying equity) Multiple comma separated items allowed for provider(s): fmp.
    ...
    def historical(
        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    'Symbol to get data for. Multiple comma separated'
                    'items allowed for provider(s): cboe, fmp, polygon,'
                    'tiingo, yfinance.;     A Security identifier'
                    '(Ticker, FIGI, ISIN, CUSIP, Intrinio ID).'
                    '(provider: intrinio)'
                )
            )
        ],
        ...
        provider: Annotated[
            Optional[Literal['cboe', 'fmp', 'intrinio', 'polygon', 'tiingo', 'yfinance']],
            OpenBBField(
                description=(
                    'The provider to use, by default None. If None, the'
                    'priority list configured in the settings is used.'
                    'Default priority: cboe, fmp, intrinio, polygon,'
                    'tiingo, yfinance.'
                )
            )
        ] = None,
...
----------
provider : str
    The provider to use, by default None. If None, the priority list configured in the settings is used. Default priority: cboe, fmp, intrinio, polygon, tiingo, yfinance.
symbol : str | list[str]
    Symbol to get data for. Multiple comma separated items allowed for provider(s): cboe, fmp, polygon, tiingo, yfinance.
...
interval : str
    ...
    (provider: cboe)
        The most recent trading day is not including in daily historical data. Intraday data is only available for the most recent trading day at 1 minute intervals.
        Choices: '1m', '1d'

    (provider: polygon)
        The numeric portion of the interval can be any positive integer. The letter portion can be one of the following: s, m, h, d, W, M, Q, Y
use_cache : bool
...
adjustment : str
    ...
    (provider: polygon)
        ...
        Choices: 'splits_only', 'unadjusted'
...
adj_close : float | None
    The adjusted close price. (provider: intrinio, tiingo)
...
>>> obb.etf.historical(symbol='SPY')
>>> # This function accepts multiple tickers.
>>> obb.etf.historical(symbol='SPY,IWM,QQQ,DJIA')
            ...
            **filter_inputs(
                provider_choices={
                    "provider": self._get_provider(
                        provider,
                        "etf.historical",
                        ('cboe', 'fmp', 'intrinio', 'polygon', 'tiingo', 'yfinance'),
                    )
                },
                standard_params={
                    "symbol": symbol,
                ...
                extra_params=kwargs,
                info={'symbol': {'cboe': {'multiple_items_allowed': True, 'choices': None}, 'fmp': {'multiple_items_allowed': True, 'choices': None}, 'polygon': {'multiple_items_allowed': True, 'choices': None}, 'tiingo': {'multiple_items_allowed': True, 'choices': None}, 'yfinance': {'multiple_items_allowed': True, 'choices': None}}, 'interval': {'cboe': {'multiple_items_allowed': False, 'choices': ['1m', '1d']}, 'intrinio': {'multiple_items_allowed': False, 'choices': ['1m', '5m', '10m', '15m', '30m', '60m', '1h', '1d', '1W', '1M', '1Q', '1Y']}, 'tiingo': {'multiple_items_allowed': False, 'choices': ['1m', '5m', '15m', '30m', '90m', '1h', '2h', '4h', '1d', '1W', '1M', '1Y']}, 'yfinance': {'multiple_items_allowed': False, 'choices': ['1m', '2m', '5m', '15m', '30m', '60m', '90m', '1h', '1d', '5d', '1W', '1M', '1Q']}}},
            )
        )
...
shares : float | str | None
    The number of shares held. (provider: fmp)
value : float | None
    The market value of the holding. (provider: fmp);
    The market value of the holding, on the 'as_of' date. (provider: intrinio)
...
country : str | None
    The country or region of the holding. (provider: intrinio)
maturity_date : date | None
    The maturity date for the debt security, if available. (provider: intrinio)
...
>>> obb.etf.holdings(symbol='XLK', date='2022-03-31')
    ...
    def info(
        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    'Symbol to get data for. (ETF) Multiple comma'
                    'separated items allowed for provider(s): fmp,'
                    'intrinio, yfinance.'
                )
            )
        ],
...
>>> obb.etf.info(symbol='SPY,IWM,QQQ,DJIA')
    ...
    def nport_disclosure(
        ...
        symbol: Annotated[
            str,
            OpenBBField(
                description='Symbol to get data for. (Fund ticker or CIK)'
            )
        ],
        year: Annotated[
            int | None,
            OpenBBField(
                description=(
                    'Reporting year of the filing. Default is the year'
        ...
        quarter: Annotated[
            ...
            OpenBBField(
                description=(
                    'Reporting quarter of the filing. Default is the'
...
currency_amount_sold : float | None
...
currency_amount_bought : float | None
...
return_type : str
    ...
    Choices for intrinio: 'trailing', 'calendar'
adjustment : str
    The adjustment factor, 'splits_only' will return pure price performance. (provider: intrinio)
    Choices for intrinio: 'splits_only', 'splits_and_dividends'
...
>>> obb.etf.price_performance(symbol='SPY,QQQ,IWM,DJIA')
...
exchange : str | None
    (provider: fmp)
        ...
        Choices: 'amex', 'nyse', 'nasdaq', 'tsx', 'euronext'
...
name : str | None
    Name of the ETF.
country : str | None
    Country where the ETF is domiciled. (provider: fmp)
exchange : str | None
    Exchange where the ETF is listed. (provider: fmp);
    The exchange MIC code. (provider: intrinio)
exchange_name : str | None
    The full name of the exchange. (provider: fmp)
market_cap : int | float | None
    Market capitalization of the ETF. (provider: fmp)
beta : float | None
    Beta of the ETF. (provider: fmp)
price : float | None
    Current price of the ETF. (provider: fmp)
last_annual_dividend : float | None
    Last annual dividend paid. (provider: fmp)
volume : int | float | None
    Current trading volume of the ETF. (provider: fmp)
...
>>> obb.etf.search(query='commercial real estate')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'exchange': {'fmp': {'x-widget_config': {'options': [{'label': 'AMEX', 'value': 'amex'}, {'label': 'NYSE', 'value': 'nyse'}, {'label': 'NASDAQ', 'value': 'nasdaq'}, {'label': 'TSX', 'value': 'tsx'}, {'label': 'Euronext', 'value': 'euronext'}]}}}},
            )
        )
