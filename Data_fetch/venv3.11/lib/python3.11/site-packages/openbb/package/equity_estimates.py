"""Autogenerated OpenBB /equity/estimates Module."""
...
class ROUTER_equity_estimates(Container):
    ...
    def analyst_search(
        ...
        analyst_name: Annotated[
            ...
            OpenBBField(
                description=(
                    'Analyst names to return. Omitting will return all'
                    'available analysts. Multiple comma separated items'
                    'allowed for provider(s): benzinga.'
                )
            )
        ] = None,
        firm_name: Annotated[
            ...
            OpenBBField(
                description=(
                    'Firm names to return. Omitting will return all'
                    'available firms. Multiple comma separated items'
                    'allowed for provider(s): benzinga.'
                )
            )
        ] = None,
...
analyst_name : str | list[str | None] | None
...
firm_name : str | list[str | None] | None
...
overall_avg_return_percentile : float | None
...
total_ratings_percentile : float | None
...
smart_score_3y : float | None
    A weighted average smart score over the last 3 years. (provider: benzinga)
success_rate_3y : float | None
...
>>> obb.equity.estimates.analyst_search(firm_name='Wedbush')
...
limit : int | None
    The number of data entries to return. Number of historical periods. (provider: fmp)
...
estimate_type : str
    ...
    Choices for intrinio: 'ebitda', 'ebit', 'enterprise_value', 'cash_flow_per_share', 'pretax_income'
...
>>> obb.equity.estimates.forward_ebitda(symbol='AAPL', fiscal_period='annual')
>>> obb.equity.estimates.forward_ebitda(symbol='AAPL,MSFT', fiscal_period='quarter')
...
calendar_period : str
    ...
    Choices for intrinio: 'q1', 'q2', 'q3', 'q4'
...
number_of_analysts : int | None
    Number of analysts providing estimates for the period.
revisions_change_percent : float | None
...
fiscal_period : str
    ...
    Choices for intrinio: 'fy', 'q1', 'q2', 'q3', 'q4'
calendar_year : int | None
    The future calendar year to retrieve estimates for. When no symbol and year is supplied the current calendar year is used. (provider: intrinio)
calendar_period : str
    The future calendar period to retrieve estimates for. (provider: intrinio)
...
page : int | None
    Page number for paginated results. Used with limit. (provider: fmp)
...
estimated_eps_high : float | None
    Estimated EPS high.
estimated_eps_low : float | None
    Estimated EPS low.
    ...
    def price_target(
        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    'Symbol to get data for. Multiple comma separated'
                    'items allowed for provider(s): benzinga, fmp.'
                )
            )
        ] = None,
        limit: Annotated[
...
action : str
    ...
    Choices for benzinga: 'downgrades', 'maintains', 'reinstates', 'reiterates', 'upgrades', 'assumes' 'initiates', 'terminates', 'removes', 'suspends', 'firm_dissolved'
...
>>> obb.equity.estimates.price_target(start_date='2020-01-01', end_date='2024-02-16', limit=10, symbol='msft', action='downgrades')
